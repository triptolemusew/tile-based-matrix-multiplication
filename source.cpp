...
void deallocate_2d_array(int **array){
	free(array[0]);
	free(array);
}
...
int main(int argc, char* argv[])
{
		...
		*/
		for (int i = 1; i < totalProcs; i++){
			if (i == 1){
				...
				MPI_Send(&temp_1dB2[0], g_rowB * sizeColPerProcsB, MPI_INT, i, 0, MPI_COMM_WORLD);
			}
			else if (i == 2){
				MPI_Send(&temp_1dA2[0], sizeRowPerProcsA * g_colA, MPI_INT, i, 0, MPI_COMM_WORLD);
		...
		for (int i = 0; i < sizeRowPerPartC; i++){
			memset(cpart[i], 0, (sizeColPerPartC * sizeof(int)));
		}
		for (int i = 0; i < sizeRowPerPartC; i++){
	...
	default: {
		...
		if (myRank == 1){
			...
			MPI_Recv(&temp_1dB2[0], g_rowB * sizeColPerProcsB, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
			int l = 0;
			for (int i = 0; i < g_rowB; i++){
				for (int j = 0; j < sizeColPerProcsB; j++){
			...
			free(temp_1dA1);
			free(temp_1dB2);
			MPI_Send(&cpart[0][0], sizeRowPerPartC * sizeColPerPartC, MPI_INT, 0, 0, MPI_COMM_WORLD);
		}
		else if (myRank == 2){
			temp_1dA2 = (int *)malloc(sizeRowPerProcsA * g_colA * sizeof(int));
			temp_1dB1 = (int *)malloc(g_rowB * sizeColPerProcsB * sizeof(int));
			MPI_Recv(&temp_1dA2[0], sizeRowPerProcsA * g_colA, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
	...
	if (myRank == 0){
		...
		MPI_Recv(&cpart[0][0], sizeRowPerPartC * sizeColPerPartC, MPI_INT, 1, 0, MPI_COMM_WORLD, &status);
		offsetCol += sizeColPerPartC;
		for (i = 0; i < sizeRowPerPartC; i++){
			for (j = 0; j < sizeColPerPartC; j++){
				matrixC[i][j + offsetCol] = cpart[i][j];
			}
		}
		MPI_Recv(&cpart[0][0], sizeRowPerPartC * sizeColPerPartC, MPI_INT, 2, 0, MPI_COMM_WORLD, &status);
		offsetCol = 0;
		offsetRow += sizeRowPerPartC;
		for (i = 0; i < sizeRowPerPartC; i++){
			for (j = 0; j < sizeColPerPartC; j++){
				matrixC[i + offsetRow][j] = cpart[i][j];
			}
		}
		MPI_Recv(&cpart[0][0], sizeRowPerPartC * sizeColPerPartC, MPI_INT, 3, 0, MPI_COMM_WORLD, &status);
		offsetCol += sizeColPerPartC;
		for (i = 0; i < sizeRowPerPartC; i++){
			for (j = 0; j < sizeColPerPartC; j++){
				matrixC[i + offsetCol][j + offsetRow] = cpart[i][j];
			}
		}
	endTime = GetCustomCurrentTime();
	processTime = endTime - startTime;
	...
	if (myRank == 0){
		...
		for (int i = 0; i < rowC; i++){
			fwrite(matrixC[i], sizeof(unsigned long long), colC, fileC);
		}
		fclose(fileC);
	...
	// 6. Cleaning up!
	if (myRank == 0){
		deallocate_2d_array(matrixC);
		deallocate_2d_array(cpart);
		free(temp_1dA1);
		free(temp_1dA2);
		free(temp_1dB1);
		free(temp_1dB2);
		...
		deallocate_2d_array(matrixA);
		deallocate_2d_array(matrixB);
		deallocate_2d_array(cpart);
	}
